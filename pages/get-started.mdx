...
- Sign in to the uploadfast dashboard.
- Create an API key (if you haven't already) by clicking
  **[add new key](https://www.uploadfast.dev/dashboard/keys)**.

Before using the service, activate your project which validates all your API keys.
...
(UploadFast is in beta, so this API is likely to change)
...
The SDK's better documented [here](https://github.com/upload-fast/js-sdk)